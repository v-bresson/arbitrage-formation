...
Sur `dashboard.php`, `quiz.php` et `admin/index.php`, le bandeau (logo + actions) reste **fixe** en haut de la page pendant le défilement (`.site-header` en `position: fixed`) ; `dashboard.php` et `quiz.php` affichent en plus un fil d'ariane fixe juste en dessous (`.breadcrumb`) — absent dans l'admin, dont le menu latéral situe déjà la section active. Comme la hauteur du bandeau varie (retour à la ligne selon la largeur d'écran ou la longueur du nom d'utilisateur), `assets/header-fix.js` la mesure réellement au chargement et à chaque redimensionnement/changement de contenu, pour réserver l'espace correspondant (`.header-spacer`) — plutôt qu'une hauteur figée en CSS, source de chevauchement.
